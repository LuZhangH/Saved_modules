...
# -*- coding: utf-8 -*-
"""
Methods appropriated from Sesia SNPknock python package,
otherwise written by me.
...
#       raise ValueError('Warning: incorrect normalization of initial probabilities! Perhaps a numerical error in fastPhase?')
    ...
    # ADDED BY NIK
#    pInit = (alpha[:,0]*alpha[:,0].T).sum(axis=0) 
    pInit = alpha[:,0].T #assumes initial state is based on alpha of first SNP
    return pInit

def writeX(X, out_file, phased=False):
    '''
